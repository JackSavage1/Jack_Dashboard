...
# Tab 1: Overview
with tab1:
    # Sidebar filters for overview tab
    with st.sidebar:
...
with tab4:
    ...
    data_tabs = st.tabs(["API Data", "Historical Data", "Linguist Data", "Staging Data"])
    
    with data_tabs[0]:
        if lass_df is not None:
            st.write("LASS Data")
            st.dataframe(lass_df, use_container_width=True)
            st.download_button(
                label="Download LASS Data",
                data=lass_df.to_csv(index=False),
                file_name="lass_data.csv",
                mime="text/csv"
            )
        
        if lep_df is not None:
            st.write("LEP Data")
            st.dataframe(lep_df, use_container_width=True)
            st.download_button(
                label="Download LEP Data",
                data=lep_df.to_csv(index=False),
                file_name="lep_data.csv",
                mime="text/csv"
            )
...
<div style='text-align: center; color: #666;'>
    ...
    Data Sources: NYC Open Data & Local Excel Files | 
    Last Updated: {}</p>
